...
BullMQ with dedicated persistent Redis, MinIO, configurable local-only Ollama,
...
- Remote OK and Himalayas collection reuse the existing repository providers and normalization
  packages. Provider warnings, cancellation and invalid results fail collection;
  they do not masquerade as successful empty searches. Select either or both;
  ...
  per source, 100 final results, 30-day window and a 60-second combined deadline.
  Himalayas scans at most 20 pages (2,000 recent postings), not its full inventory.
  A selected-source failure fails the whole attempt; partial-success persistence
  and detailed per-source progress remain pending. Remotive is not enabled until
...
- Completed-search JSON downloads through `GET /api/searches/:id/export` and the
  ...
  validated run ID. Version 1 contains `schemaVersion`, `runId`, `request` and
